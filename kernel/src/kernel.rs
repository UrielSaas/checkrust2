...
impl Kernel {
    ...
    pub fn new(processes: &'static CoreLocal<MapCell<StaticSlice<Option<&'static dyn process::Process>>>>) -> Self {
        Kernel {
            processes,
    ...
    pub(crate) fn processid_is_valid(&self, processid: &ProcessId) -> bool {
        self.processes.with(|ps| ps.map_or(false, |ps| ps.get(processid.index).map_or(false, |p| {
